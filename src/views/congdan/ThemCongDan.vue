<template>
  <MainLayout>
    <div>
      ...
      <h1 class="title">Bổ sung công dân</h1>
          ...
          <form @submit.prevent="handleSubmit">
            <div class="grid">
              ...
              <div class="field">
                ...
                <span v-if="errors.cccd" class="error-message">{{ errors.cccd }}</span>
                <input v-model="model.cccd" type="text" />
              </div>
            ...
            <button type="submit" class="btn-submit">Tạo công dân</button>
          </form>
        </div>
...
onMounted(async () => {
  try {
    // Load danh sách tỉnh - SỬA DÒNG NÀY
...
async function handleSubmit() {
  ...
  try {
    ...
    formData.append('Cccd', model.value.cccd)
    ...
    await congdanService.create(formData)
    toastStore.success('Thông tin công dân đã được lưu thành công!')
